				...
				add("listassetpool", [](const params& params)
					{
						if (params.size() < 2) 
						{
							...
							for (auto i = 0; i < game::qos::XAssetType::ASSET_TYPE_COUNT; i++)
							{
						...
						{
							const auto type = static_cast<game::qos::XAssetType>(atoi(params.get(1)));
							if (type < 0 || type >= game::qos::XAssetType::ASSET_TYPE_COUNT)
							{
								console::info("Invalid pool passed must be between [0, %d]\n", game::qos::XAssetType::ASSET_TYPE_COUNT - 1);
								return;
							}
							...
							fastfiles::enum_assets(type, [type, filter](const game::qos::XAssetHeader header)
								{
									auto asset = game::qos::XAsset{ type, header };
