...
namespace game_console
{
	namespace
	{
		...
		void draw_input()
		{
			...
			//draw_input_text_and_over("QOS: >", color_title);
			...
			else if (matches.size() == 1)
			{
				...
				if (dvar)
				{
					...
					draw_hint_text(0, dvars::Dvar_ValueToString(dvar, dvar->current),
					...
					draw_hint_text(1, dvars::Dvar_ValueToString(dvar, dvar->reset),
					...
					const auto line_count_ = dvar->type == game::dvar_type::enumeration ? dvar->domain.enumeration.stringCount + 1 : 1;
			...
			else if (matches.size() > 1)
			{
				...
				for (size_t i = 0; i < matches.size(); i++)
				{
					...
					if (dvar)
					{
						draw_hint_text(static_cast<int>(i), dvars::Dvar_ValueToString(dvar, dvar->current),
							dvars::con_inputDvarValueColor->current.vector, offset);
		...
		void draw_output_text(const float x, float y, output_queue& output)
		{
			...
			for (auto i = 0; i < con.visible_line_count; i++)
			{
				...
				game::R_AddCmdDrawText(output.at(index).data(), 0x7FFF, console_font, x, y + offset, 1.0f, 1.0f, 0.0f, color_white, 0);
			}
		}
		...
		void draw_output_window()
		{
			...
			game::R_AddCmdDrawText("QOS", 0x7FFFFFFF, console_font, x,
	...
	public:
		...
		{
			scheduler::loop(draw_console, scheduler::pipeline::main);
			...
			scheduler::once([]()
				{
					dvars::con_inputBoxColor = dvars::Dvar_RegisterVec4("con_inputBoxColor", "color of console input box", 0.2f, 0.2f, 0.2f, 0.9f, 0.0f, 1.0f, game::dvar_flags::saved);
					dvars::con_inputHintBoxColor = dvars::Dvar_RegisterVec4("con_inputHintBoxColor", "color of console input hint box", 0.3f, 0.3f, 0.3f, 1.0f, 0.0f, 1.0f, game::dvar_flags::saved);
					dvars::con_outputBarColor = dvars::Dvar_RegisterVec4("con_outputBarColor", "color of console output bar", 0.5f, 0.5f, 0.5f, 0.6f, 0.0f, 1.0f, game::dvar_flags::saved);
					dvars::con_outputSliderColor = dvars::Dvar_RegisterVec4("con_outputSliderColor", "color of console output slider", 0.3f, 0.7f, 0.3f, 1.0f, 0.0f, 1.0f, game::dvar_flags::saved);
					dvars::con_outputWindowColor = dvars::Dvar_RegisterVec4("con_outputWindowColor", "color of console output window", 0.25f, 0.25f, 0.25f, 0.85f, 0.0f, 1.0f, game::dvar_flags::saved);
					dvars::con_inputDvarMatchColor = dvars::Dvar_RegisterVec4("con_inputDvarMatchColor", "color of console matched dvar", 1.0f, 1.0f, 0.8f, 1.0f, 0.0f, 1.0f, game::dvar_flags::saved);
